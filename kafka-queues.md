...
* **Labelized Tickets**: Labelized messages after external processing.
...
* **Labelized Ticket Dead Letter Queue**: Failed messages from the Labelized Ticket topic (those that failed to transition from formatting to labeling).
...
* The KPI Aggregator service consumes labelized tickets from the `tickets_labelized` topic.
